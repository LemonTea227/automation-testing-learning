...
describe('TC3 - Login User with incorrect email and password', () => {
    ...
    it('8th - Verify that "Your email or password is incorrect!" is visible', async () => {
        let titleSelector = '.login-form p';
        await page.waitForSelector(titleSelector, {
            visible: true,
        })
            .then(() => {
        ...
        expect((await (await (await page.$(titleSelector)).getProperty('textContent')).jsonValue()).trim()).toBe('Your email or password is incorrect!');
